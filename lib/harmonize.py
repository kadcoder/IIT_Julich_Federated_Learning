    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith('fc') and name != 'fc4.bias': #name.startswith('fc') and 
            ...
            harmonized_silo_1 = torch.from_numpy(
                np.mean(data_harmonized[:, :federated_epochs], axis=1).reshape(param.shape).astype(np.float32)
            ).to(param.device)
...
def harmonize_localglobal(
    ...
    for name, param in model.named_parameters():
        ...
        if name.startswith('fc') and name != 'fc4.bias': #name.startswith('fc') and 
            ...
            aggregated_gradients[name] = harmonized_local + harmonized_global # Adjust the scaling factor as needed
