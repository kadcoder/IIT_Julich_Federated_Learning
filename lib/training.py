...
from config import INIT_LR, DEVICE, WEIGHT_DECAY, GRADIENT_CLIP
from harmonize import harmonize_localglobal
from sklearn.model_selection import train_test_split


def compute_gradients(model, criterion, X_scaled, y, total_samples):
    model.train()
    ...
    grads = {name: param.grad.clone().detach() * (X_scaled.shape[0] / total_samples)
...
def train_localmodel(X_silo, y_silo, model, total_samples, epochs,
                     best_val_loss):

    # change to the train_test_split function
    X_train_raw, y_train, X_val_raw, y_val = split_data(X_silo, y_silo)
    ...
    # Modified optimizer configuration
    optimizer = torch.optim.SGD(lmodel.parameters(), lr=INIT_LR, momentum=0.9,
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Alignment loss with global model
            alignment_loss = 0
            for (l_param, g_param) in zip(lmodel.parameters(),
        ...
        if val_loss < min(best_val_loss):
            ...
            best_model = copy.deepcopy(lmodel.state_dict())
    ...
    # Compute gradients on local training data
    local_gradients = compute_gradients(lmodel, criterion, X_train,
                                        y_train, total_samples)
    avg_train_loss = sum(train_losses) / len(train_losses)
    avg_val_loss = sum(val_losses)/len(val_losses)
...
def train_globalmodel(gmodel, global_train_path, total_samples,
                      FINE_TUNE_EPOCHS=10):

    X_global, y_global, silo = preprocess(global_train_path)

    # TODO: change to the train_test_split function
    ...
    # This scheduler reduces the learning rate by a factor of 0.5 if
    # the average loss does not improve for 2 epochs.
...
def train_centralmodel(train_path, max_epochs):
    ...
    X_train_inner, y_train, X_val_raw, y_val = train_test_split(X, y,
                                                                test_size=0.3,
                                                                random_state=42)    # noqa
    ...
    for epoch in range(max_epochs):
        ...
        if val_loss < min(best_val_loss):
            best_val_loss = val_loss
            # TODO: check that the model is copied correctly
