...
from lib.data_utils import preprocess                                                               # noqa
from lib.training import train_globalmodel, train_localmodelgrads                                   # noqa
from lib.evaluation import evaluate_model, predict_age, compute_validation_loss                                              # noqa
from lib.config import DEVICE
from lib.harmonize import harmonize_localsilos
#%%
# #####################
...
initial_results_path = os.path.join(
    project_dir, 
    f'Experiments_1_2_eNKI/post-H-m2/results_post-H-m2/post-H-m2_l{federated_epochs}.csv'
)

ensure_dir(initial_results_path)  # Ensures only the directory exists
...
best_global_loss = float('inf') #global_dict['validation_loss']  # Initialize with the global validation loss
...
csv_loss_path = os.path.join(project_dir, f'Experiments_1_2_eNKI/post-H-m2/results_post-H-m2/train_val_loss_localsilo_l{federated_epochs}.csv')
with open(csv_loss_path, mode='w', newline='') as f2:
    ...
    for epoch in range(Global_epoch):
        ...
        for silo in local_silos:
            ...
            grads, train_losses, val_losses, best_val_loss, best_model = train_localmodelgrads(
        ...
        aggregated_gradients = harmonize_localsilos(model_global,silo_grads,federated_epochs)
...
results_age_path = os.path.join(project_dir, 'Experiments_1_2_eNKI/post-H-m2/results_post-H-m2/predictions')
...
plot_loss_curves(loss_history, project_dir, 'Experiments_1_2_eNKI/post-H-m2/results_post-H-m2/')  # Plotting function
