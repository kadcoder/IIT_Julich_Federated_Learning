...
from training import train_globalmodel,train_localglobal
...
def post_H_m3(base_dir):
    ...
    f =open(f'results_post-H-m3_l15.txt', 'w')
    dir_path = os.path.join(base_dir,'post-H-m3/train_val_losses')
    ...
    for epochs in MAX_EPOCHS:
        ...
        f2 = open(f'train_val_losses/post-H-m3_train_val_loss_localsilo_l15_{epochs}.txt','w')
        silo_scalers = {}
        ...
        for epoch in range(epochs):  # Global Epochs
            ...
            for silo in silos: # Local Epochs
                ...
                grads,train_loss,val_loss,scaler = train_localglobal(train_path, model_global, total_samples)
            ...
            with torch.no_grad():
                for name, param in model_global.named_parameters():
                    if not name.startswith('bn') and name !='fc3.bias':
