...
from lib.config import DEVICE, set_random_seed  # noqa
...
test_files = {
    silo: os.path.join(silodata_path, f"{silo}/Test_{silo}.csv") for silo in all_silos
}
...
loss_history = {
    ...
    "eNKI": {
        "train": [],
        "val": [],
    },  # using 'train' to store avg_global_loss from train_globalmodel
}
...
global_path = os.path.join(silodata_path, "eNKI/Train_eNKI.csv")

# Warmstart the global model on the central data (eNKI)
model_global, train_losses, val_losses, global_dict = train_globalmodel(
    model_global,  # noqa
    global_path,  # noqa,
...
initial_results_path = os.path.join(
    project_dir,
    f"Experiments_1_2_eNKI/pre-H/results_pre-H/pre-H_l{federated_epochs}.csv",
)
os.makedirs(
    os.path.dirname(initial_results_path), exist_ok=True
...
with open(initial_results_path, "w", newline="") as csvfile:
    ...
    writer.writerow(
        ["Initial global model loss on central data", global_dict["avg_global_loss"]]
    )
...
best_val_loss_silo = {
    silo: float("inf") for silo in local_silos
}  # Initialize best validation loss for each silo

best_model_silo = {
    silo: None for silo in local_silos
}  # Initialize best model for each silo

# Usage in training loop
best_global_loss = float("inf")  # global_dict['validation_loss'] #float('inf')
...
for silo in local_silos:  # Local silos data
...
csv_loss_path = os.path.join(
    project_dir,
    f"Experiments_1_2_eNKI/pre-H/results_pre-H/train_val_loss_localsilo_l{federated_epochs}.csv",
)
with open(csv_loss_path, mode="w", newline="") as f2:
    writer = csv.writer(f2)
    writer.writerow(["Epoch", "Silo", "Train_MAE", "Val_MAE"])  # header

# Federative learning loop
for epoch in range(Global_epoch):
    aggregated_gradients = None
    sum_loss = 0.0

    for silo in local_silos:
        print(f"Training on {silo}")
        # Train local model on silo data
        ...
        if aggregated_gradients is None:
            ...
            aggregated_gradients = {
                key1: g1 + g2
                for (key1, g1), (key1, g2) in zip(
                    aggregated_gradients.items(), grads.items()
                )
            }
    ...
    # ToDO: sabe the model that minimize the validation loss of all silos

    # Store in global dict if needed
    # best_global_loss = (best_global_loss + sum_loss) / len(all_silos) #best_global_loss #(best_global_loss + sum_loss) / len(all_silos)
...
with open(initial_results_path, "a", newline="") as csvfile:
    ...
    for silo, test_path in test_files.items():
        ...
        writer.writerow(
            [f"Test on {silo}", f"Epochs: {Global_epoch}", f"Test MAE: {avg_loss}"]
        )
    ...
    for silo, test_path in test_files.items():
        if silo == "eNKI":
            ...
            predict_age(best_model_global, test_path, results_age_path, global_scaler)
        else:
            scaler_silo = silo_scalers[silo]
            avg_loss = evaluate_model(test_path, best_model_global, scaler_silo)
            predict_age(best_model_global, test_path, results_age_path, scaler_silo)
        writer.writerow(
            [f"Test on {silo}", f"Epochs: {Global_epoch}", f"Test MAE: {avg_loss}"]
        )
...
plot_loss_curves(
    loss_history, project_dir, "Experiments_1_2_eNKI/pre-H/results_pre-H/"
)  # Plotting function


# ToDO: test
# best silo model (for enki it will be the global model)
# best global model (minimize the loss in eNKI) for all silos (for enki will be the same as before)
# last trained global model for all silos
# the best global model that minimizes the validation loss of all silos.
