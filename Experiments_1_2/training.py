...
def train_localmodel(train_path, model, total_samples, epochs=15):
    ...
    return local_gradients, avg_train_loss, avg_val_loss , scaler
...
def train_localglobal(train_path,model,total_samples,epochs=15):
    ...
    #model = m.AgePredictor(X_train.shape[1]).to(DEVICE)
    ...
    #test_loss = evaluate_localmodel(test_path,lmodel,scaler)

    globalmodel = copy.deepcopy(lmodel)
    global_train_path = f'Experiments_1_2/silo_Datasets/eNki/Train_eNki.csv'
    X_global, y_global, silo = preprocess(global_train_path)
    ...
    return aggregated_grads, avg_train_loss, val_loss,scaler