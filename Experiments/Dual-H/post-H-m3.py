...
from lib.data_utils import preprocess                                                               # noqa
from lib.training import train_localglobalgrads, train_globalmodel                                       # noqa
...
current_lr = 1e-3 #8e-4 #5e-4 #1e-3
gradient_clip = 1.0
momentum = 0.8
set_parameters()
# Configuration
...
global_dict['best_global_loss'] = best_global_loss  # Store the best model in the global dict
...
with open(csv_loss_path, mode='w', newline='') as f2:
    ...
    for epoch in range(Global_epoch):
        ...
        for silo in local_silos:
            ...
            total_samples+= X_silo_current.shape[0]

            grads, train_losses, val_losses, best_val_loss, best_model = train_localglobalgrads(
