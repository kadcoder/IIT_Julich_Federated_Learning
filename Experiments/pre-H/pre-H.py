...
#from lib.config import DEVICE                           # noqa
#%%
...
initial_results_path = os.path.join(project_dir, f'Experiments_1_2_eNKI/pre-H/results_pre-H/pre-H_l{federated_epochs}.csv')
...
for silo in local_silos:  # Local Epochs
    ...
    X_train_silo[silo] = X
    y_train_silo[silo] = y
    silo_kl[silo], silo_mmd[silo] = (0.5, 0.3) if silo == 'SALD' else (0.3, 0.5)
    

csv_loss_path = os.path.join(project_dir, f'Experiments_1_2_eNKI/pre-H/results_pre-H/train_val_loss_localsilo_l{federated_epochs}.csv')
...
with open(csv_loss_path, mode='w', newline='') as f2:
    ...
    for epoch in range(Global_epoch):
        ...
        best_global_loss, best_model_global,best_epoch = compute_validation_loss(
            gmodel=model_global,
...
results_age_path = os.path.join(project_dir, 'Experiments_1_2_eNKI/pre-H/results_pre-H/predictions')
...
with open(initial_results_path, 'a', newline='') as csvfile:
    ...
    writer.writerow([f'Final Evaluation Results on best global model : {best_epoch}'])
...
#plot_loss_curves(loss_history, project_dir, 'Experiments_1_2_eNKI/pre-H/results_pre-H/')  # Plotting function
